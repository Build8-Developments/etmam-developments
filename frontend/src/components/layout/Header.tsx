...
export default function Header({
  ...
  return (
    <>
      ...
      >
        <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            ...
            <Link
              href={`/${locale}`}
              className="flex items-center transition-transform duration-300 [@media(hover:hover)]:hover:scale-105"
            >
              {logo ? (
                <Image
            ...
            <div className="flex items-center gap-2 lg:gap-3">
              ...
              <Button
                ...
                variant="primary"
                size="sm"
                className="hidden lg:inline-flex whitespace-nowrap text-sm font-bold tracking-wide transition-all duration-300 [@media(hover:hover)]:hover:scale-[1.08] active:scale-[0.98]"
              />

              {/* Mobile Menu Button - Enhanced Animation */}
              <button
                onClick={toggle}
                className="lg:hidden relative min-w-[44px] min-h-[44px] flex items-center justify-center rounded-full bg-gradient-to-br from-gray-50 to-gray-100 hover:from-green-50 hover:to-green-100 border-2 border-gray-200 hover:border-green-500 transition-all duration-300 [@media(hover:hover)]:hover:scale-105 active:scale-95 shadow-sm hover:shadow-md focus:outline-none focus:ring-2 focus:ring-green-500/30 group touch-manipulation"
                aria-label={getTranslation(
                  "accessibility",
                  "menuToggle",
                  language
              ...
              >
                ...
                <span
                  className={`absolute inset-0 rounded-full bg-green-500 opacity-0 ${isOpen ? "animate-ping" : ""
                    }`}
                ></span>
                ...
                <div className="relative w-6 h-5 flex flex-col justify-center items-center">
                  <span
                    className={`absolute w-6 h-0.5 bg-gray-700 group-hover:bg-green-600 rounded-full transition-all duration-300 ease-in-out ${isOpen ? "rotate-45 translate-y-0" : "-translate-y-2"
                      }`}
                  ></span>
                  <span
                    className={`absolute w-6 h-0.5 bg-gray-700 group-hover:bg-green-600 rounded-full transition-all duration-300 ease-in-out ${isOpen ? "opacity-0 scale-0" : "opacity-100 scale-100"
                      }`}
                  ></span>
                  <span
                    className={`absolute w-6 h-0.5 bg-gray-700 group-hover:bg-green-600 rounded-full transition-all duration-300 ease-in-out ${isOpen ? "-rotate-45 translate-y-0" : "translate-y-2"
      ...
      {/* Mobile menu - Simplified and made RTL-safe for mobile */}
      <div
        id="mobile-nav"
        dir={language === "ar" ? "rtl" : "ltr"}
        className={`${isOpen
          ? "opacity-100 pointer-events-auto translate-y-0 visible"
          : "opacity-0 pointer-events-none -translate-y-4 invisible hidden"
          } lg:hidden transition-all duration-300 ease-out fixed top-[70px] left-0 w-full px-4 ${Z_INDEX_CLASSES.MOBILE_MENU
          }`}
      >
        <div className="mx-auto max-w-lg rounded-3xl bg-white shadow-2xl ring-1 ring-black/10 overflow-hidden backdrop-blur-sm">
          {/* Menu Header */}
