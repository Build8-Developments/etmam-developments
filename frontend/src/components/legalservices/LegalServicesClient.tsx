...
export default function LegalServicesClient({ companies }: LegalServicesClientProps) {
          ...
          {companies.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {companies.map((company, index) => (
                <AnimatedSection key={company.id} animation="slideInUp" delay={index * 0.1}>
                  <Link href={`/legalservices/${company.id}`} className="block h-full">
                    <div className={`
                      ...
                      <div className="p-8 relative">
                        ...
                        {company.servicesCount > 0 && (
                          ...
                          ">
                            ...
                            <span>
                              {language === 'ar' 
                                ? `${company.servicesCount} ${company.servicesCount === 1 ? 'خدمة' : 'خدمات'}`
                                : `${company.servicesCount} ${company.servicesCount === 1 ? 'service' : 'services'}`
                              }
                            </span>
                          </div>
                        )}
            ...
            <AnimatedSection animation="fadeIn">
              <div className="text-center py-24">
                ...
                <h3 className="text-3xl font-bold text-gray-800 mb-4">
                  {language === 'ar' ? 'لا توجد جهات متاحة حالياً' : 'No Entities Available'}
                </h3>
