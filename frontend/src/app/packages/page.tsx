    ...
    <div className="min-h-screen bg-white">
          ...
          <div 
            ...
            style={{
              backgroundImage: packagesPageData?.Hero?.backgroundImage?.url 
                ? `url(${process.env.NEXT_PUBLIC_STRAPI_API_URL}${packagesPageData.Hero.backgroundImage.url})`
                : 'url(/bgabout.png)',
          ...
          <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10" style={{ pointerEvents: "auto" }}>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
              ...
              <div className="text-white">
                ...
                >
                  {getLocalizedValue(packagesPageData?.Hero?.title) || content.hero.title[language]}
                </h1>
                ...
                <p 
                  ...
                  style={{ fontFamily: 'var(--font-almarai)' }}
                >
                  {getLocalizedValue(packagesPageData?.Hero?.subtitle) || content.hero.description[language]}
                </p>
                
                <div className="flex flex-col sm:flex-row gap-4">
                  {packagesPageData?.Hero?.primaryButton && (
                    <Link 
                      href={packagesPageData.Hero.primaryButton.href || "/contact"}
                      className="bg-white text-green-600 hover:bg-gray-100 px-8 py-4 rounded-full font-semibold transition-colors"
                      style={{ fontFamily: 'var(--font-almarai)' }}
                    >
                      {getLocalizedValue(packagesPageData.Hero.primaryButton.label) || content.hero.buttons.primary[language]}
                    </Link>
                  )}
                  {packagesPageData?.Hero?.secondaryButton && (
                    <Link 
                      href={packagesPageData.Hero.secondaryButton.href || "/offers"}
                      ...
                      style={{ fontFamily: 'var(--font-almarai)' }}
                    >
                      {getLocalizedValue(packagesPageData.Hero.secondaryButton.label) || content.hero.buttons.secondary[language]}
                    </Link>
                  )}
                </div>
              </div>
              ...
              <div className="relative">
                <div className="grid grid-cols-2 gap-4">
                  {/* Package Stats Cards */}
                  {packagesPageData?.Hero?.stats?.map((stat: any, index: number) => (
      ...
      <section className="py-16 lg:py-24">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {(packagesPageData?.packages && packagesPageData.packages.length > 0 
              ? packagesPageData.packages 
              : packages).map((pkg: any, index: number) => {
              // Transform Strapi package or use mock package
              const packageTitle = getLocalizedValue(pkg.title) || (pkg.name && typeof pkg.name === 'object' ? pkg.name[language] : pkg.name) || '';
              const packagePrice = typeof pkg.price === 'string' ? pkg.price : (pkg.price && typeof pkg.price === 'object' ? pkg.price[language] : String(pkg.price || ''));
              const packageFeatures = pkg.feature?.map((f: any) => getLocalizedValue(f.feature)) || pkg.features?.map((f: any) => typeof f === 'string' ? f : getLocalizedValue(f)) || [];
              ...
              return (
                <div 
                  ...
                  className={`relative bg-white rounded-2xl shadow-lg p-8 ${
                    isPopular ? 'ring-2 ring-green-500 transform scale-105' : ''
                  }`}
                >
                  {isPopular && (
                  ...
                  <ul className="space-y-4 mb-8">
                    {packageFeatures.map((feature: string, featureIndex: number) => (
      ...
      <AnimatedSection animation="scaleIn" delay={100}>
        <CTASection 
          ...
          buttonText={getLocalizedValue(packagesPageData?.contact_card?.description)}
        />
      </AnimatedSection>
      ...
      <AnimatedSection animation="fadeIn" delay={150}>
        <PartnersSection 
          partners={packagesPageData?.partners?.partners}
        />
      </AnimatedSection>
